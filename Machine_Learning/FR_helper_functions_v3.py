...
def train(model: torch.nn.Module,
    ...
    x_int_sample, x_diff_sample,x_diff2_sample, _ = sample_batch
    writer.add_graph(model, (x_int_sample.to(device), x_diff_sample.to(device),x_diff2_sample.to(device)))
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        for x_int, x_diff, x_diff2, y_batch in train_loader:
            x_int, x_diff, x_diff2, y_batch = x_int.to(device), x_diff.to(device),x_diff2.to(device), y_batch.to(device)
            preds = model(x_int, x_diff, x_diff2)
        ...
        with torch.inference_mode():
            for x_int, x_diff,x_diff2, y_batch in test_loader:
                x_int, x_diff,x_diff2, y_batch = x_int.to(device), x_diff.to(device),x_diff2.to(device), y_batch.to(device)
                preds = model(x_int, x_diff,x_diff2)
