...
def train(model: torch.nn.Module,
    ...
    x_int_sample, x_diff_sample, x_diff2_sample, x_fft_sample, _ = sample_batch
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        for x_int, x_diff, x_diff2, x_fft, y_batch in train_loader:
            x_int, x_diff, x_diff2, x_fft, y_batch = (x_int.to(device), 
                                                      x_diff.to(device),
                                                      x_diff2.to(device), 
                                                      x_fft.to(device),
                                                      y_batch.to(device))
            preds = model(x_int, x_diff, x_diff2, x_fft)
        ...
        with torch.inference_mode():
            for x_int, x_diff,x_diff2, x_fft, y_batch in test_loader:
                x_int, x_diff,x_diff2, x_fft, y_batch = (x_int.to(device), 
                                                        x_diff.to(device),
                                                        x_diff2.to(device),
                                                        x_fft.to(device), 
